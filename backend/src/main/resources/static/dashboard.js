...
class LeaveDashboard {
    ...
    async loadHeaderUserInfo() {
        try {
            ...
            if (response.ok) {
                ...
                // Set friendly greeting message
                const greetingElement = document.getElementById('userGreeting');
                if (greetingElement) {
                    const safeName = (userProfile.fullName || 'there').split(' ')[0];
                    greetingElement.textContent = `Hey ${safeName}! Let's make today a great day for planning leave.`;
                }
    ...
    showNotification(message, type = 'info') {
        // Add human-friendly emoji prefix
        if (type === 'success' && !message.startsWith('🎉')) {
            message = `🎉 ${message}`;
        } else if (type === 'error' && !message.startsWith('⚠️')) {
            message = `⚠️ ${message}`;
        }
